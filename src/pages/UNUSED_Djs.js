    ...
    <div >
        <div className='column is-12'>
            {djs.filter(dj=> dj.region === region).map(it=>
                ...
                onClick={()=>setDj(it)}>{it.nickname?it.nickname:(it.name)}
                </Button>
            )}    
        </div>
        {dj.email?
            <div className="columns is-centered" style={{color:COLORS.WHITE, width:'100vw', textAlign:'left', padding:40}}>
                <div className="column is-6 has-white-text">
                    <div style={{textAlign:'center'}}>
                        <strong style={{fontSize:36, fontWeight:'bold'}}>{dj.nickname?dj.nickname:(dj.name)}</strong>
                        <p/><p/>
...
export default () => {
    ...
    const handleReply = reply => {
        if (reply.status === 'OK') {
            ...
            setDjs(reply.result.filter(it=>it.isDiskjockey==1))
        } else {
    ...
    const uniqueRegions = uniqueList(djs.map(it => it.region))
    const regions = REGIONS.filter(it =>uniqueRegions.includes(it))

    return(
        <div style={styles.container}>
                <div style={{width:'100vw', textAlign:'center'}}>
                    <h2 style={{color:COLORS.YELLOW}}>DJ</h2>
